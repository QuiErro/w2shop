...
create table goods(
  	...
  	add_time datetime not null default 0 comment '添加时间',
  	index cat_id(cat_id),
...
#创建商品品牌表
create table brand(
	brand_id smallint not null auto_increment primary key comment '商品品牌ID',
	...
	sort_order tinyint not null default 50 comment '商品品牌排序依据',
	is_show tinyint not null default 1 comment '是否显示，默认显示'
)engine=MyISAM charset=utf8;

#创建分类表
create table category(
	cat_id smallint not null auto_increment primary key comment '商品类别ID',
	cat_name varchar(30) not null default '' comment '商品类别名称',
	parent_id smallint not null default 0 comment '商品类别父ID',
...
#创建数据库
create database if not exists w2shop charset utf8;
...
#插入一条记录作为管理员
insert into w2_admin(admin_name,password,email) values('admin','21232f297a57a5a743894a0e4a801fc3','admin@itcast.cn');
insert into w2_admin(admin_name,password,email) values('admin','admin','admin@itcast.cn');



#类别部分，先插入部分数据，做测试
insert into w2_category(cat_name,parent_id) values('手机类型',0);
insert into w2_category(cat_name,parent_id) values('充值卡',0);
insert into w2_category(cat_name,parent_id) values('手机配件',0);
insert into w2_category(cat_name,parent_id) values('CDMA手机',1);
insert into w2_category(cat_name,parent_id) values('3G手机',1);
insert into w2_category(cat_name,parent_id) values('iphone 10',5);
insert into w2_category(cat_name,parent_id) values('联通手机充值卡',2);
insert into w2_category(cat_name,parent_id) values('移动手机充值卡',2);
insert into w2_category(cat_name,parent_id) values('耳机',3);
insert into w2_category(cat_name,parent_id) values('电池',3);